...
if(isset($_POST["tab"])){	unset($_SESSION["defaultTab"]);	$defaultTab= $_POST["tab"]; unset($_POST["tab"]); echo "trying= ".$defaultTab;}
else if(isset($_SESSION["defaultTab"])){	$defaultTab= $_SESSION["defaultTab"]; unset($_SESSION["defaultTab"]); echo "value= ".$defaultTab;}
else	$defaultTab= "component";
//$defaultTab= $_SESSION["defaultTab"];
// This chunk of CODE is for Default: when the page will be loaded
/*if($defaultTab)//isset($_SESSION["defaultTab"])
{
	//echo $defaultTab;
	//$main= $_SESSION["main"];
	//$defaultTab= $_SESSION["defaultTab"];
	...
	else {	$main= $_SESSION["main"]; }
	if(isset($_POST["tab"])){ $defaultTab= $_POST["tab"]; }
	else{ $defaultTab= $_SESSION["defaultTab"]; }
}
else
{*/
	...
	if(isset($_POST["main"])){ $main = $_POST["main"]; }
	else if(isset($_SESSION["main"])) {	$main= $_SESSION["main"]; }
	else
	{
		$domain = $_SESSION["domain"];
		$l = mysql_query("SELECT * FROM objects_fwdc WHERE domain='$domain' ORDER BY RAND() LIMIT 1");
	...
	//if(isset($_POST["tab"])){ $defaultTab= $_POST["tab"]; }
	// Pre-set for Relation TAB
	$_SESSION["ma"] = $_SESSION["main"];
	
	$mainRel = $_SESSION["ma"];
	$result = mysql_query("SELECT * FROM `temp` WHERE main='$mainRel' AND count<=10 ORDER BY count DESC LIMIT 7");
	$art = array();
	$i=0;
...
?>
		...
		<div class="tab-pane fade <?php if($defaultTab=='relation') echo 'in active'; ?>" id="relation">
			...
			<div id="opone">
				<p style="">
					<li id="awesome">Required in Manfacture of <?php echo $mainRel; ?></li>
					<li id="awesome">Essential for <?php echo $mainRel; ?>'s primary purpose</li>
				</p>
			</div>
			<div id="optwo">
				<p style="">
			...
			<div id="opthree">
				<p style="">
					<li id="awesome">Present in <?php echo $mainRel; ?> but </li>
					<li id="awesome">Useful in other purposes not primary</li>
				</p>
			</div>
			<div id="opfour">
				<p style="">
					<li id="awesome">Not a part of <?php echo $mainRel; ?> but </li>
					<li id="awesome">Required to fulfill its primary purpose.</li>
				</p>
			</div>
			<div id="opfive" >
				<p style="">
					<li id="awesome">Subtype of <?php echo $mainRel; ?></li>
				</p>
			</div>
			</div>
			<div>
